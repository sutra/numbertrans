...
public class SourceLineCounter extends CommentProcessor {
	 ...
	 */
	public int countLinesRecursively(File root, boolean includeHiddenDirs, boolean skipImports,
		...
		for (final File file : files)
			nDirectoryLines += countLinesInFile(file, skipImports);
		...
		for (final File dir : directories)
			if (includeHiddenDirs || !dir.getName().startsWith("."))
				if (Arrays.binarySearch(ignoreDirectories, dir.getName()) < 0)
					nDirectoryLines += countLinesRecursively(dir, includeHiddenDirs, skipImports,
							ext);
	...
	public int countLinesInFile(File file, boolean skipImports) throws IOException {
		nFiles++;
		nFileLines = 0;
		...
		while ((line = in.readLine()) != null) {
			...
			if (skipImports && line.startsWith("import"))
				continue;
	...
	public static void main(String... args) throws IOException {
		boolean SKIP_IMPORTS = true;

		SourceLineCounter c = new SourceLineCounter("sphinx4", "log4net-1.2.10", "TestTools",
				"wxWidgets", "ui");
		int nTotalLines = c.countLinesRecursively(new File(args[0]), false, SKIP_IMPORTS, ".java",
				".cs", ".cpp", ".h", ".c");
		int nFiles = c.getFileCount();
		// int nTotalLines = c.countLinesInFile(new File(args[0]));
		System.out.println(nTotalLines + " non-comment, non-blank source lines in " + nFiles
				+ " files for " + args[0]);
	}
