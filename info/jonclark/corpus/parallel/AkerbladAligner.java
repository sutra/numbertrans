...
import net.sourceforge.akerblad.Akerblad;

public class AkerbladAligner implements ParallelCorpusAlignmentRun {

	private final String pathToChampollion;
	private final String dictionaryFile;
	private final String stopListFile;
	private final double ratio;
	private static final boolean disallow3 = true;
	private final Akerblad akerblad;

	private static final Logger log = LogUtils.getLogger();

	public AkerbladAligner(Properties props, String runName, String corpusName) throws IOException {
		...
		throw new Error();
//		this.akerblad =
//				new Akerblad(new AkerbladConfig(stopListFile, dictionaryFile, false, disallow3,
//						false, ratio, LengthMetric.CHARS));
	}
		...
		try {
			while (iterator.hasNext()) {
				...
				if (StringUtils.countOccurances(inE.getWholeFile(), '\n') > MAX_LINES
					...
					// skip articles with more than 100 lines; they're probably
					// garbage
					;
				} else {
					akerblad.doAlignment(inE.getFile().getAbsolutePath(),
							inF.getFile().getAbsolutePath(), alignments.getAbsolutePath());
	...
	public static void main(String... args) throws Exception {
		...
		AkerbladAligner akerblad = new AkerbladAligner(props, null, null);
		...
		for (int i = 0; i < filesX.length; i++) {
			...
			// alignmentFileName);
			akerblad.akerblad.doAlignment(filesX[i].getAbsolutePath(), fileY.getAbsolutePath(),
