...
public class HtmlToChampollionConverter {
	...
	private static String filterNonContent(String html) {
		...
		// wired EN
		html = HtmlUtils.extractFirstOf(html, "<div id=\"article_text\"",
				"</div>", true);
		// hotwired JP
		html = HtmlUtils.extractFirstOf(html, "<!-- ARTICLE/-->",
				"<!-- /ARTICLE-->", true);
		...
		html = HtmlUtils.extractFirstOf(html, "<!--content_start-->",
				"<!--content_stop-->", true);
		// www.linuxplanet.com
		...
		// www.winplanet.com
		html = HtmlUtils.extractFirstOf(html, "<!--start_reviews -->",
				"<!--end_reviews-->", true);
		// no optimization for www.clickz.com
		// www.eweek.com
		html = HtmlUtils.extractFirstOf(html,
				"<td align=\"left\" class=\"Article_Content\"", "</td>", true);
		// news.com.com
	...
	public static void main(String... args) throws Exception {
		...
		for (int i = 0; i < htmlFiles.length; i++) {
			...
			final String[] oldArr = new String[] { "<br>", "</h1>", "</h2>",
					"</h3>", "</h4>", "</h5>" };
			...
			// Step 6: save file
			final String fileName = StringUtils.substringBefore(htmlFile
					.getName(), ".html")
					+ ".txt";
			FileUtils.saveFileFromString(new File(htmlFile.getParentFile(),
					fileName), plainText);
			...
			if (i % 100 == 0) {
				System.out
						.println("Processing file "
								...
								+ est
										.getEstimatedCompetionTimeFormatted(htmlFiles.length
												- i));
			}
		}
	}
